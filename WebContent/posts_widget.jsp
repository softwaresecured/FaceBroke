			...
			<div class="panel panel-default" id="${p.id}">
				<div class="panel-heading">
					...
					<c:if test="${!p.creator.id.equals(p.wall.user.id)}">
						<c:set var="header_content" scope="page" value="   <span class='glyphicon glyphicon-triangle-right name-sep'></span>   <a href='wall?user_id=${p.wall.user.id}'><img src='image?id=${p.wall.user.profilePicture.id}' alt='User profile picture' class='img-rounded profile-img-post'>${p.wall.user.fname} ${p.wall.user.lname}</a>" />
					</c:if>
					<h4>
						<a href="wall?user_id=${p.creator.id}"><img src='image?id=${p.creator.profilePicture.id}' alt='User profile picture' class='img-rounded profile-img-post'>${p.creator.fname} ${p.creator.lname}</a>${header_content}</h4>
				</div>
	...
	<nav aria-label="..." class="col-md-8 col-md-offset-2">
		<ul class="pager">
			<li class="previous ${back_disabled}">
				<a href="${back_path}" class="${back_disabled}"><span aria-hidden="true">&larr;</span> Newer</a>
			</li>
			<li class="next">
				<a href="${forward_path}">Older <span aria-hidden="true">&rarr;</span></a>
			</li>
			<li><c:out value="" /></li>
		</ul>
	</nav>
